...
### v3.2 (2026-03-13)
- 🐛 **修复**：视频在 1 分钟左右自动暂停的问题（扩展拦截 30s/60s/90s 试用倒计时）
- 🔍 **修复**：顶部搜索栏消失的问题（改用 CSS 隐藏登录弹窗，不再拦截 miniLogin 脚本）
- 🔄 **修复**：连播视频时评论区不更新的问题（新增 SPA 路由监听，自动切换评论）
- ⌨️ **增强**：支持空格键暂停视频（之前只支持鼠标点击）
- 🛡️ **优化**：改进 DOM 拦截策略，减少对页面其他功能的影响
